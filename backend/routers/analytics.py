"""routers/analytics.py - Vertical-aware dashboard statistics."""
from fastapi import APIRouter, Depends
from sqlalchemy import func
from sqlalchemy.orm import Session

from database import Campaign, EmailLog, Lead, LeadStatus, User, get_db
from routers.auth import get_current_user
from verticals import entitlements_for_user, plan_for_user
...
def overview(
    ...
    ready_to_send = db.query(Lead).filter(Lead.user_id == uid, Lead.status == LeadStatus.pending, Lead.email != None, Lead.email != "").count()  # noqa: E711
    ...
    return {
        "vertical": getattr(current_user, "vertical", "business_growth"),
