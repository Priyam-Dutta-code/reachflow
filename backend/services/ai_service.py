...
Reads the user's encrypted Groq API key from their profile, falling back to GROQ_API_KEY.
"""
import hashlib
import json
import random
...
def _cache_key(company: str, name: str, user_id: str) -> str:
    return hashlib.md5(f"{user_id}|{company.lower()}|{name.lower()}".encode("utf-8")).hexdigest()
...
def generate_email_for_lead(lead, user) -> tuple[str, str]:
    ...
    ck = _cache_key(company, name, user.id)
    if ck in _cache:
...
Sender is {user.sender_name}, sent an initial email a few days ago to {name} at {company}.
Reference the previous email naturally. Not desperate. One soft CTA. Mention CV is still attached.
